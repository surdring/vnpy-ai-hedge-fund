...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_menu(self) -> None:
        ...
        # AI Agent menu
        ai_menu: QtWidgets.QMenu = bar.addMenu(_("AI Agent"))

        self.add_action(
            ai_menu,
            _("启动 AI 对冲基金"),
            get_icon_path(__file__, "ai_hedge_fund.ico"),
            self.open_ai_hedge_fund
        )
    ...
    def open_ai_hedge_fund(self) -> None:
        """
        Open AI Hedge Fund configuration widget.
        """
        engine = self.main_engine.get_engine("AiHedgeFund")
        if engine:
            ...
            QtWidgets.QMessageBox.warning(
                self,
                _("AI Hedge Fund"),
                _("AI Hedge Fund 引擎未加载，请先加载 AiHedgeFundApp"),
                QtWidgets.QMessageBox.StandardButton.Ok
            )

    def open_ai_agent_config(self) -> None:
        """
        Open AI Agent configuration panel.
        """
        engine = self.main_engine.get_engine("AiHedgeFund")
        if engine:
    ...
    def open_ai_signal_monitor(self) -> None:
        """
        Open AI signal monitoring panel.
        """
        engine = self.main_engine.get_engine("AiHedgeFund")
        if engine:
            engine.get_status()
